...
void HUD::drawHearts() {
	for (int i = 0; i < 10; i++) {
		m_renderer.drawQuad(
			(float)m_window.getWidth() / 2.0f - (HUDSprites::HEART_WIDTH * 5.0f / 2.0f) - HUDSprites::HOTBAR_WIDTH * 5.0f / 2.0f + i * HUDSprites::HEART_WIDTH * 5 + 16,
			(float)m_window.getHeight() * 0.85f - 2,
			HUDSprites::HEART_WIDTH * 4.0f, HUDSprites::HEART_HEIGHT * 4.0f,
